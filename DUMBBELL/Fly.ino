...
void loop() 
{
  ...
  if(roll >= -20 && roll <= 20 && pitch >= -20 && pitch <= 20 && yaw >= -180 && yaw <= 105&& gyroX >= -150 && gyroX <= 150)
  {
    saveState = 1;        
  }
  if(roll >= 85 && roll <= 115 && pitch >= -20 && pitch <= 20 && yaw >= -180 && yaw <= 105&& gyroX >= -150 && gyroX <= 150)
  {
    if(saveState == 1)
    {
      CountSave += 1;
...
//  }
    ...
    M5.Lcd.printf("StringCount =  %S",stringcount);
     delay(1);
 }
